...
def jacobian(x):
    #la matrice jacobienne contient les dérivées partielles de la fonction F(x) par rapport à chacune des variables. Dans notre cas, la matrice jacobienne est une matrice 1x3 contenant les coefficients
    return np.array([[10, 8, 1]])


def newton_raphson_multi_variable(x0, max_iter=100, tol=1e-6):
    x = np.array(x0)
    #À chaque itération, la méthode met à jour les valeurs des variables en soustrayant le produit de la matrice pseudo-inverse de la jacobienne et de la fonction F(x) aux valeurs actuelles des variables.
...
"""
Le problème est que la solution fournie par cette méthode peut ne pas satisfaire la contrainte de x1, x2 et x3 étant compris entre 0 et 1. 

la méthode de Newton-Raphson ne prend pas en compte les contraintes sur les variables lors de la recherche d'une solution. Cette méthode cherche simplement à trouver un zéro (ou une racine) de la fonction F, sans considérer si les valeurs de x1, x2 et x3 respectent les contraintes données (ici, être compris entre 0 et 1).
"""
